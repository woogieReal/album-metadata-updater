...
- [ ] `DirectoryTree` 위젯으로 전체 파일시스템 탐색 가능하도록 구성
- [ ] 하단에 `현재 폴더 사용하기` 버튼 배치
- [ ] `DirectoryTree`에서 디렉터리 선택 시 현재 선택 경로 상태 갱신

### 4-2. 상태 2: 대상 폴더 선택 후 (사용 버튼 클릭 시)

- [ ] 선택된 폴더 하위에서 `.mp3` 파일만 필터링하여 목록 추출
- [ ] 최상단 `전체 선택/해제` 체크박스 구현 (토글 시 하위 전체 연동)
- [ ] 추출된 MP3 파일명 목록 및 개별 체크박스 렌더링 (기본값: 모두 선택)
- [ ] `현재 폴더 사용하기` 버튼을 `설정할 메타데이터 고르기` 버튼으로 교체

### 4-3. MetadataScreen 전환 (설정 버튼 클릭 시)

- [ ] 체크된 MP3 파일 목록을 수집하여 `MetadataScreen`으로 전달
- [ ] `app.push_screen(MetadataScreen(...))` 방식으로 화면 전환
...
- [ ] 우상단 `닫기` 버튼 구현 (클릭 시 `app.pop_screen()`으로 탐색 화면 복귀)
- [ ] 하단 `메타데이터 수정하기` 버튼 배치

### 5-2. 메타데이터 설정 목록 영역

- [ ] 최상단 `전체 항목 선택/해제` 체크박스 구현 (기본값: 모두 선택)
- [ ] `앨범명` 입력 필드 + 체크박스
...
- [ ] `트랙명` 항목 + 체크박스 (입력창 없음, 자동 정제 적용 예정 안내 표기)
- [ ] `트랙번호` 항목 + 체크박스 (입력창 없음, 파일명 오름차순 순번 적용 예정 안내 표기)
