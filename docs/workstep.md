...
- [ ] 수정 버튼 클릭 시 체크된 항목만 선별하여 `MetadataService` 호출
  - 체크된 필드(`앨범명`, `아티스트`, `연도`, `트랙명`, `트랙번호`)만 ID3 태그에 반영
- [ ] 성공 시: 성공 알림 메시지(Toast/Modal) 표시 후 탐색 화면으로 자동 복귀
- [ ] 실패 시: 실패 알림 메시지 표시 후 메타 화면 상태 유지

---

## Phase 7. 통합 테스트 및 마무리

- [ ] 전체 플로우 통합 수동 테스트 (디렉터리 탐색 → 파일 선택 → 메타데이터 설정 → 수정)
- [ ] 엣지 케이스 처리
